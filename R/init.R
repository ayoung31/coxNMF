#' @export
init <- function(X,M,y,delta,k,alpha,WtX,lambda,eta,ninit=5,imaxit=10,warmup=1,verbose,norm.type){
  ...
  for(i in 1:ninit){
    ...
    fit0 <- optimize_loss(X=X,M=M,H0=H0,W0=W0,beta0=beta0,y=y,delta=delta,
                          alpha=0,lambda=0,eta=0,maxit=warmup,WtX=WtX,verbose=verbose,norm.type=norm.type)
    fit <- optimize_loss(X=X,M=M,H0=fit0$fit$H,W0=fit0$fit$W,beta0=fit0$fit$beta,y=y,delta=delta,
                         alpha=alpha,lambda=lambda,eta=eta,maxit=max(imaxit-warmup,0),WtX=WtX,verbose=verbose,norm.type=norm.type)
    loss <- fit$loss$loss
    if(loss < loss_best){
      loss_best=loss
      fit_best=fit
