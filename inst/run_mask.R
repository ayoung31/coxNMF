...
load("TCGA_PAAD_gencode_filtered_full.RData")
X=Xtrain
y = rep(0,ncol(X))
delta = rep(0,ncol(X))
...
metrics = foreach(pa=1:nrow(params), .inorder = FALSE, .errorhandling = 'pass', .combine = 'rbind') %dopar% {
  ...
  for(init in 1:ninit){
    ...
    fit_curr = coxNMF::run_coxNMF(X,y,delta,k,a, l,e,H0,W0,beta0,M,tol=1e-5,
  ...
  fit_cox = coxNMF::run_coxNMF(X,y,delta,k,a, l,e,best$H,best$W,best$beta,M,tol=1e-6,
